...
fetch(`http://api.weatherapi.com/v1/current.json?key=${apiKey}&q=Atlanta`).then(function(response){
    ...
    fetch(`http://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=Atlanta`).then(function(response){
        return response.json();
            ...
            forecastHigh.innerHTML = `High for the Day | ${data.forecast.forecastday[0].day.maxtemp_f}`;
            forecastLow.innerHTML = `Low for the Day | ${data.forecast.forecastday[0].day.mintemp_f}`;
            forecastAvg.innerHTML = `Today's Average | ${data.forecast.forecastday[0].day.avgtemp_f}`;
